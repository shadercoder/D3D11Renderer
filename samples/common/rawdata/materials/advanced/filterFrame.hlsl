...
float3 filter(float2 uv, float3 right, float3 up, float3 forward) {
	...
	for (int i = 0; i < NUM_SAMPLES; ++i) {
		...
		col += InputTexture.Sample(SamplerLinear, sampDir.xy).rgb;
	}
...
float4 psMain(PsIn psIn) : SV_TARGET0 {
	...
	float3 col = InputTexture.Sample(SamplerLinear,
		psIn.Uvs * float2(1,-1) * (1.0 + pow(Roughness, 2) / 10000.0)).rgb;
	//col *= float3(1,0,Roughness);
