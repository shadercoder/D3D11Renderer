...
PsOut psMain(PsIn psIn) {
	...
	psOut.Face0 = float4(panoramaToCube(psIn.Uvs, nz, py, px), 0.0);
	psOut.Face1 = float4(panoramaToCube(psIn.Uvs, pz, py, nx), 0.0);
	psOut.Face2 = float4(panoramaToCube(psIn.Uvs, px, nz, py), 0.0);
	psOut.Face3 = float4(panoramaToCube(psIn.Uvs, px, pz, ny), 0.0);
	psOut.Face4 = float4(panoramaToCube(psIn.Uvs, px, py, pz), 0.0);
	psOut.Face5 = float4(panoramaToCube(psIn.Uvs, nx, py, nz), 0.0);
	return psOut;
}